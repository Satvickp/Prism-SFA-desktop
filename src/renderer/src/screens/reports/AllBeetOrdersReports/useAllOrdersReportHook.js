...
import { orderCallStatus, orderMedium, ReportCallType, ReportIdType, ReportType } from "./FilterComponent";
...
const useAllOrdersReportHook = () => {
  ...
  const [selectedReportTypeLevel, setSelectedReportTypeLevel] = useState(ReportType[0]);
  const [selectedReportIdLevel, setSelectedReportIdLevel] = useState(ReportIdType[0]);
  const [selectedReportCallLevel, setSelectedReportCallLevel] = useState(ReportCallType[0]);
  const [selectedOrderStatus, setSelectedOrderStatus] = useState(orderCallStatus[0]);
  const [selectedOrderMedium, setSelectedOrderMedium] = useState(orderMedium[0]);
  ...
  const columns = useMemo(() => {
    if (selectedReportTypeLevel?.value === "beet") {
      return [
        ...
        {
          name: "Total Sales",
          selector: (row) => (row?.totalSales ? `₹ ${Math.round(row.totalSales)}` : "NA"),
          sortable: true,
        },
        {
          name: "Total Orders",
