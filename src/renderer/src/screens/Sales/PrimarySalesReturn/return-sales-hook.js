...
import { dispatchSecondarySalePaginatedOrder } from '../../../redux/features/order/secondary-sale'
...
export const useReturnSalesHook = (isPrimary) => {
  const [loading, setLoading] = useState(-1)
  const dispatch = useDispatch()
  const Sales = useSelector((state) => state[isPrimary ? 'PrimarySales' : 'SecondarySales'])
  ...
  async function helperFunctionSales(page) {
    ...
    try {
      ...
      if (isPrimary) {
        dispatch(
          page === 0 ? dispatchPrimarySaleSetOrder(resp) : dispatchPrimarySalePaginatedOrder(resp)
        )
      } else {
        dispatch(
          page === 0
  ...
  const sales_col = useMemo(() => {
    const common_cols = [
      {
        name: 'Member',
        selector: (row) => row?.orderResponseList?.[0]?.memberName || 'No details',
        sortable: true
      },
      {
        name: 'Date',
    ...
    if (!isPrimary) {
      common_cols.splice(0, 0, {
        name: 'Status',
        selector: (row) => (
          <strong style={getOrderStatusStyle(row?.orderResponseList?.[0]?.status)}>
    ...
    if (!isClient) {
      common_cols.splice(0, 0, {
        name: 'Client',
        selector: (row) => row?.orderResponseList?.[0]?.clientName || 'No details',
        sortable: true
      })
    }
    common_cols.splice(0, 0, {
